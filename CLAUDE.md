...
- **Session 9 Fixes:** Added `model_id` column to drafts table. generate-draft Inngest function now stores actual model used (glass-box). Draft editor reads model from draft data (not localStorage). ideas.list enriches items with latest draft status per idea. IdeaCard shows status badge + "View Draft" button when draft exists. SourceRail Edit/Remove text replaced with pencil/trash icons. localStorage model IDs validated against known MODELS list. ideas.list cache invalidated after generate.
- **Next:** Prompt Studio UI → LinkedIn E2E publish test (production) → Phase 6 (7-dim grading)
